...
<!-- if we mention stuff like "application logic, business rules, and  objectives" we should probably expand on what that is exactly in our app. I think i fixed this shit slighty - Jackson-->

To enable UniBasement to meet the described architecturally significant requirements, a layered architecture comprised of three layers was implemented. The highest layer was the Presentation Layer, which was comprised of our front-end, built with Next.js using Tailwind. These technologies were chosen as they are industry standard which enables easier contribution. Components within the front-end were designed to be generic and extensible where possible whilst maintaining a maximum of 300 lines to minimise complexity. Unit tests were implemented through the integration tests to ensure that functionality was as expected. The Business layer was implemented to handle API requests, basic calculations used in data processing and sending requests to the database. The Database Layer was implemented via PostgreSQL as it’s an industry-standard relational database management system (RDBMS). PostgreSQL ensures data security and provides robust features for managing data. However, because of the use of an ORM in the Business Layer, the Database Layer can easily be changed to any other RDBMS if necessary with little impact on development or the product.

# TODO @tristan change this cause it is just wrong smh
...
<!-- scaling is mentioned here but might be worth expanding on how that helps reliability vs scalability which isn't one of our QAs. Still nice to talk about in minimal depth ig - Jackson -->
...
<!-- Some of the later shit is just words for being words, mainly second last line. We should do this cause it is needed, but it was needed so we didnt do it? -->
